...
const client = new RealtimeClient({
	apiKey:
		"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",
...
client.updateSession({
	tools: [
		...
		},
		{
			name: "file_upload",
			description:
				"Upload a CSV file containing URLs, parse them with PapaParse, scrape each URL with Firecrawl, and save to memory using mem0ai",
			parameters: {
				type: "object",
				properties: {
					fileContent: {
...
export function ConsolePage() {
	...
	console.log(
		"🚀 ~ file: ConsolePage.tsx:117 ~ ConsolePage ~ webData:",
		webData,
	);
	...
	useEffect(() => {
		...
		client.addTool(
			{
				name: "file_upload",
				description:
					"Parse csv file from upload for urls and scrape all of them.",
				parameters: {
					type: "object",
					properties: {
						fileContent: {
			...
			async ({
				fileContent,
				urlColumnName,
			}: { fileContent: string; urlColumnName: string }) => {
				...
				for (const url of urls) {
					try {
						...
						if (scrapedData?.markdown && scrapedData.metadata) {
							const {
								metadata: { description, keywords, title },
								markdown,
							}: any = scrapedData;
							console.log(
								"🚀 ~ file: ConsolePage.tsx:621 ~ scrapedData:",
								scrapedData,
							);
							...
							// Update UI state
							setScrapeBucket((prevBucket: any) => [
								...prevBucket,
								{ ...scrapedData },
							]);
				...
				return {
					ok: true,
					message: `Processed ${urls.length} URLs successfully`,
				};
			},
		);
		...
		client.addTool(
			{
				...
				description:
					"Navigate to certain page and scrapes data using @Firecrawl.",
				parameters: {
					type: "object",
					properties: {
						url: {
			...
			async ({ url }: { [key: string]: any }) => {
				...
				if (data?.markdown && data.metadata) {
					const {
						metadata: { description, keywords, title, ogImage, sourceUrl },
						markdown,
						links,
					}: any = data;
					...
					setScrapeBucket((scrapeBucket: any) => [
						...scrapeBucket,
						{ ...data },
					]);
					const newState = webData?.length
						? [
								...webData,
								{
		...
		client.addTool(
			...
			async ({ url, search }: { [key: string]: any }) => {
				...
				const newData: any = webData?.length
					? [
							...webData,
							{
						...
						{webData?.length
							? webData.map(
									({
										markdown,
										screenshot,
									}: { markdown: string; screenshot: string }) => (
										<div key={screenshot}>
