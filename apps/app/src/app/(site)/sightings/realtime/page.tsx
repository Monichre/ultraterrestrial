import { SightingsGlobe } from "@/features/sightings/sightings-globe";
import { SightingsVisualization } from "@/features/sightings/uap-dashboard/sightings-visualization";
import {
	UAPSightingSchema,
	type ValidatedUAPSighting,
} from "@/services/sightings/uap-sighting";
...
async function getSightings() {
	try {
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_APP_URL}/api/internal/uap-sightings`,
			{
				headers: {
...
async function getMilitaryBases() {
	try {
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_APP_URL}/api/internal/military-bases`,
			{
...
async function getUFOPosts() {
	try {
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_APP_URL}/api/internal/ufo-posts`,
			{
		...
		<div className="flex flex-col gap-8">
			...
			<div className="h-[600px] w-full relative bg-card rounded-lg overflow-hidden">
				...
				>
					<SightingsGlobe
						...
						militaryBases={
							militaryBases || { type: "FeatureCollection", features: [] }
						}
			...
			<div className="container mx-auto px-4 py-8">
				<h1 className="text-2xl font-bold mb-6">
					Realtime UAP Sightings in New Jersey
				</h1>
