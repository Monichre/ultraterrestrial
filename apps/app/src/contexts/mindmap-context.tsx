...
interface MindMapContextType {
	...
	// Node management
	addUserInputNode: (params: { input: string; user: any; position?: XYPosition }) => Node;
	findConnections: (node: Node) => Node[];
	addNextEntitiesToMindMap: (source: Node) => { groupNode: Node; groupNodeChildren: Node[] };
	...
	createSearchResultsLayout: (params: { sourceNode: Node; searchResults: any[] }) => { searchResultNodes: Node[]; searchResultEdges: Edge[] };
	
	// Entity management
	retrieveEntitiesFromStore: (model: keyof DatabaseSchema) => any[];
	addMindmapChildNode: (params: { parentNode: Node; type: string; childNode: any; position: XYPosition }) => { newNode: Node; newEdge: Edge };
	addMindMapGroupNode: (params: { connectionNode: Node; model: string; nodeType?: string; groupId: string; position: XYPosition }) => void;
	...
	// Database loading
	loadNodesFromTableQuery: (params: { type: string; searchResults: any[]; searchTerm: string }) => Promise<{ childNodes: { groupNode: Node; groupNodeChildren: Node[] } }>;
...
export const MindMapProvider = ({
	...
	const toggleLocationVisualization = useCallback(() => setShowLocationVisualization(prev => !prev), []);
	const toggleKeepLoaded = useCallback(() => setKeepLoadedOnMap(prev => !prev), []);
	const updateMindMapInstance = useCallback((instance: any) => setMindMapInstance(instance), []);
	...
	const createRootNodeEdge = useCallback((rootNodeChildNode: any, source: any) => {
	...
	const createRootNodeEdges = useCallback((rootNodeChildNodes: Node[], source: any) => {
	...
	// Layout functions
	const createSearchResultsLayout = useCallback(({ sourceNode, searchResults }: any) => {
	...
	const positionAdjacentNodesRadially = ({ sourceNode, adjacentNodes }: { sourceNode: any; adjacentNodes: any }) => {
		const searchResultNodes: any = [];
		const searchResultEdges: any = [];
	...
	const addConnectionNodesFromSearch = useCallback(({ source, searchResults }: AddConnectionNodesFromSearchParams) => {
	...
	const addUserInputNode = useCallback(({ input, user, position }: any) => {
		const newNode = {
	...
	// Group node creation
	const createGroupNodeLayoutWithoutRootNode = useCallback(({ groupId, childNodes }: any) => {
	...
	const renderUserInputResultsLayout = useCallback(({ groupId, sourceNode, results }: any) => {
	...
	const detectNodeOverlap = useCallback((node: { id: string }): Node[] => {
	...
	const addNextEntitiesToMindMap = useCallback((source: any) => {
		const { type: nodeType, data: { type: model } } = source;
	...
	const getRootNodeChildren = useCallback(async (type: any) => {
		...
		// Update the graph
		const initialRootNodes = getNodes().filter((node: any) => node.id !== source).concat([rootNode]);
	...
	const loadNodesFromTableQuery = useCallback(async ({ type, searchResults, searchTerm }: any) => {
	...
	// Connection finding
	const findConnections = useCallback((node: any) => {
		const { id } = node;
	...
	// Viewport adjustment
	const adjustViewport = useCallback(({ x, y, zoom = 0, duration = 800 }: FlowPosition) => {
