...
const PARTICLES_PER_CURVE = 1200 // Increased particle count since Bun can handle it better
const CURVES = 2
const RADIUS = 4
...
function Helix( { mousePosition }: any ) {
  let points: any = useRef()
  ...
  useFrame( ( state ) => {
    ...
    if ( particles.current ) {
      ...
      particles.current.rotation.y += ( targetRotationY - particles.current.rotation.y ) * 0.05
      particles.current.rotation.x += ( targetRotationX - particles.current.rotation.x ) * 0.05
      ...
      for ( let i = 0; i < PARTICLES_PER_CURVE * CURVES; i++ ) {
        ...
        const angle = t * EIGHT_PI + ( Math.floor( i / PARTICLES_PER_CURVE ) * Math.PI )
  ...
  // Optimized shader with Bun-compatible syntax
  ...
  return <points ref={particles} geometry={particleGeometry} material={shaderMaterial} />
}
