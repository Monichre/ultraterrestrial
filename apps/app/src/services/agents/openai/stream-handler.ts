...
import { searchDatabase } from '@/services/agents/disclosure/tools/search-database'
...
export class AssistantStreamEventHandler extends EventEmitter {
  ...
  async handleRequiresAction( data: any, runId: any, threadId: any ) {
    console.log( "🚀 ~ file: stream-handler.ts:47 ~ AssistantStreamEventHandler ~ handleRequiresAction ~ data:", data )

    try {
      const toolOutputs =
        data.required_action.submit_tool_outputs.tool_calls.map( async ( toolCall: { function: { name: string; arguments: string }; id: any } ) => {
          if ( toolCall.function.name === "search_database" ) {
            const { table, search_terms: searchTerms, search_fields: searchFields } = JSON.parse( toolCall.function.arguments )
