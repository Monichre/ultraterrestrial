

const VAULTS = [
  'https://www.theblackvault.com/documentarchive/category/the-fringe/ufo-phenomena/',
  'https://www.cia.gov/readingroom/search/site?f%5B0%5D=im_field_taxonomy_keyword%3A477',
  'https://vault.fbi.gov/UFO/',
  'https://www.nsa.gov/Helpful-Links/NSA-FOIA/Frequently-Requested-Information/Unidentified-Flying-Objects-UFOs/',
]
// 'https://drive.google.com/drive/folders/1rgRvJf2qwDypmFaMa67s2405qyMbUe__',
export const EXTERNAL_RESOURCES = [
  'https://www.ufos.wiki/',
  'https://anomalien.com',
  'https://enigmalabs.io/',
  'https://www.ufoexplorations.com/',
  'https://www.anomalist.com/',
  'https://anomalyarchives.org/',
  'https://inplainsight-book.com/',
  'https://www.thinkaboutitdocs.com/',
  'https://nufohrc.org/',
  'https://www.mufog.co.uk/',
  'https://uapswiki.wpenginepowered.com',
  'https://newparadigminstitute.org/',
  "https://howandwhys.com/",
  'https://bestufovideos.com',
  'https://thedebrief.org/category/uap/',
  'http://www.theblackvault.com/casefiles/category/ufos',
  'http://www.cufos.org/index.html',
  'https://thedebrief.org/category/uap',
  'http://www.educatinghumanity.com',
  'http://www.isaackoi.com',
  'https://www.letstalkthis.com/ufos',
  'https://www.reddit.com/r/UFOs',
  'https://nationalufocenter.com',
  'http://www.nicap.org',
  'http://www.nuforc.org',
  'http://www.project1947.com',
  'http://www.stealthskater.com/UFO.htm',
  'https://www.uaptheory.com',
  'https://ufologie.patrickgross.org/sys/text.htm',
  'https://ufopanel.com',
  'http://www.ufostalker.com',
  'https://www.thedrive.com/the-war-zone',
  'http://www.tarrdaniel.com/documents/ufology.html',
  'https://midimagic.sgc-hosting.com/ufopage.htm',
  'http://www.noufors.com/english.html',
  'http://www.saturdaynightuforia.com',
  'http://www.ufocasebook.com',
  'http://www.ufoencounters.co.uk',
  'http://ufoevidence.org',
  'https://web.archive.org/web/20180913014349/http://www.ufoevidence.org',
  'http://ufotheater.com/ufo-channel-reviews/ufo-black-list',
  'http://www.ufowatchdog.com',
  'https://www.ufojoe.net',
  'https://omnitalkradio.weebly.com',
  'https://web.archive.org/web/20040204184527/http://www.nidsci.org/researchnews.html',
  'https://www.oig.dhs.gov/foia/reading-room',
  'https://www.dhs.gov/us-visit-documents-foia-electronic-reading-room',
  'https://vault.fbi.gov/reading-room-index',
  'https://www.cia.gov/readingroom/home',
  'https://www.twz.com',
  'https://www.disclosurediaries.com/timeline',
  'https://www.disclosurediaries.com',
  'https://www.isaackoi.com',
  'https://www.ufos.wiki',
  'https://anomalien.com',
  'https://enigmalabs.io',
  'https://www.ufoexplorations.com',
  'https://www.anomalist.com',
  'https://anomalyarchives.org',
  'https://www.theblackvault.com/documentarchive/category/the-fringe/ufo-phenomena',
  'https://www.cia.gov/readingroom/search/site?f%5B0%5D=im_field_taxonomy_keyword%3A477',
  'https://vault.fbi.gov/UFO',
  'https://www.nsa.gov/Helpful-Links/NSA-FOIA/Frequently-Requested-Information/Unidentified-Flying-Objects-UFOs',
  'https://www.history.com/news/ufo-sightings-credible-modern',
  'https://inplainsight-book.com',
  'https://www.thinkaboutitdocs.com',
  'https://nufohrc.org',
  'https://drive.google.com/drive/folders/1rgRvJf2qwDypmFaMa67s2405qyMbUe__',
  'https://www.mufog.co.uk',
  'https://www.ufos.wiki',
  'https://anomalien.com',
  'https://enigmalabs.io',
  'https://www.ufoexplorations.com',
  'https://www.anomalist.com',
  'https://anomalyarchives.org',
  'https://inplainsight-book.com',
  'https://www.thinkaboutitdocs.com',
  'https://nufohrc.org',
  'https://www.mufog.co.uk',
  'https://uapswiki.wpenginepowered.com',
  'https://newparadigminstitute.org',
  'https://howandwhys.com',
  'https://ufos-scientificresearch.blogspot.com',
  'https://mindsublime.blogspot.com',
  "https://www.stardrive.org/"

]

export const CORE_DATA_SOURCES = [

]
export const YOUTUBE_RESOURCES = [
  'www.youtube.com/@JesseMichels',
  'www.youtube.com/@TheBlackVaultOriginals',
  'https://www.youtube.com/@JeremyCorbell/podcasts',
  'https://www.youtube.com/playlist?list=PL6PrA6lo8rJLRExhMvX6wKyNzy0hr_QM3',
  "https://www.youtube.com/@JeremyCorbell"

]
export const RSS_FEEDS = [
  {
    site: 'https://www.newsnationnow.com/podcasts-newsnation/reality-check-with-ross-coulthart/', rss: 'https://www.newsnationnow.com/podcasts-newsnation/reality-check-with-ross-coulthart/rss', name: 'Reality Check with Ross Coulthart',
  },
  { site: 'https://www.theblackvault.com/documentarchive/feed/', name: 'The Black Vault', rss: 'https://www.theblackvault.com/documentarchive/feed/' },
  { site: 'https://www.theblackvault.com/casefiles/category/ufos/', name: 'The Black Vault Case Files', rss: 'https://www.theblackvault.com/casefiles/category/ufos/feed/' },
  { site: 'https://howandwhys.com/', name: 'How and Why', rss: 'https://howandwhys.com/feed/' },
]





