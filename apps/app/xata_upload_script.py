...
def convert_photo_to_xata_format(photo_data):
    ...
    pass
    base64_content = photo_data.replace('XataFile.fromBase64("', '').rstrip('")')
...
def upload_batch(entries, batch_size=50):
    ...
    for i in range(0, total_entries, batch_size):
        ...
        print(f"\nProcessing batch {i//batch_size + 1} ({i+1} to {min(i+batch_size, total_entries)} of {total_entries})")
        ...
        for entry in batch:
            try:
                ...
                print(f"Failed to upload entry: {entry.get('name', 'Unknown')}")
                print(f"Error: {str(e)}")
        
        pass
        print(f"Progress: {successful}/{total_entries} entries uploaded successfully")
        if failed:
