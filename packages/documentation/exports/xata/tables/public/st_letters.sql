create function st_letters(letters text, font json DEFAULT NULL::json) returns geometry
    immutable
    parallel safe
    cost 500
    language plpgsql
...
alter function st_letters(text, json) owner to supabase_admin;

grant execute on function st_letters(text, json) to postgres;

grant execute on function st_letters(text, json) to anon;

grant execute on function st_letters(text, json) to authenticated;

grant execute on function st_letters(text, json) to service_role;
