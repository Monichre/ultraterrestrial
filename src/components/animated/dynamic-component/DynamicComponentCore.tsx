...
const DynamicComponentSizePresets: Record<SizePresets, Preset> = {
  [SIZE_PRESETS.RESET]: {
    width: 150,
    aspectRatio: 1,
    borderRadius: 20,
...
const DynamicComponentProvider: React.FC<DynamicComponentProviderProps> = ({
  children,
  ...
  const contextValue = {
    ...
    setSize,
    scheduleAnimation,
    presets: DynamicComponentSizePresets,
  }
...
const useDynamicComponentSize = () => {
  const context = useContext(BlobContext)
  if (!context) {
    throw new Error(
      'useDynamicComponentSize must be used within a DynamicComponentProvider'
    )
  }
  return context
}
...
const useScheduledAnimations = (
  ...
  const { scheduleAnimation } = useDynamicComponentSize()
...
const DynamicComponentContainer = ({ children }: { children: ReactNode }) => {
  return (
...
const DynamicComponentContent = ({
  ...
  const { state, presets } = useDynamicComponentSize()
...
const DynamicContainer = ({ className, children }: DynamicContainerProps) => {
  const willChange = useWillChange()
  const { state } = useDynamicComponentSize()
...
export {
  ...
  DynamicTitle,
  DynamicDescription,
  DynamicComponentCore,
  SIZE_PRESETS,
  stiffness,
  DynamicDiv,
  damping,
  DynamicComponentSizePresets,
  BlobContext,
  useDynamicComponentSize,
