...
export function InputWithVanishAnimation({
  placeholders,
  onSubmit,
  type,
}: {
  ...
  return (
    ...
    >
      ...
      <button
        disabled={!value}
        type='submit'
        className='absolute right-2 top-1/2 z-50 -translate-y-1/2 h-8 w-8 rounded-full duration-300 ease-in-out hover:shadow transition duration-200 flex items-center justify-center'
      >
        {/* bg-[radial-gradient(circle_at_50%_0%,hsl(var(--primary)/10%)_0%,transparent_60%)] dark:hover:shadow-[0_0_20px_hsl(var(--primary)/10%)]  */}
        {/* backdrop-blur-xl transition-[box-shadow]  */}
        <motion.svg
