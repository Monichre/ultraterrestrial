...
const useMapStore = create<RFState>((set, get) => ({
  ...
  loadFromStorage: () => {
    ...
    let selectedPalette = palettes.find(
  ...
  onNodesChange: (changes: Array<NodeChange>) => {
    const sureChanges = changes.filter(
  ...
  applyPalette: (palette: PaletteElement) => {
    ...
    graph.forEachNode((nodeId, attributes) => {
      const parentNodeAttributes = attributes.data.parentId
        ? graph.getNodeAttributes(attributes.data.parentId)
        : null

      const parentColor = parentNodeAttributes
        ? colorMap[parentNodeAttributes.id]
        : null
